...
public class MapPractice2 {
    public static void main(String[] args) {
        ...
        List<Map<String, Object>> listOfMaps = new ArrayList<>();//when you have multiple map pbjects
        ...
        for (Map<String, Object> eachMap : listOfMaps) {
            ...
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {//we need another loop for iterating map
            ...
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {
                if (eachEntry.getKey().equals("hired_date")) {
                    if (((LocalDate) eachEntry.getValue()).getYear() == 2022) {
                        System.out.println(eachMap.get("name"));//nora iskender umran cikiyor 2022 de ise girmisler
                    }
                }
            }
        }
        ...
        System.out.println("---------------------------------------------------");
//  list has index number so you can use index number in that get method/ it return the map ands you call replace method
        listOfMaps.get(0).replace("name", "Vasyl");//put da olr ama replace better bastaki key
