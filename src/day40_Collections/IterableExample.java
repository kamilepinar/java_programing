...
public class IterableExample {
    public static void main(String[] args) {
        ...
        /*
        for (Integer each : list) { //this is give exceptions
            if(each < 5){
                list.remove(each);
            }
        }
      */
        ...
        for (int i = 0; i < list.size(); i++) { // this is not enough
            if (list.get(i) < 5) {
                list.remove(i);
            }
        }
        ...
        list5.removeIf( p -> p < 5);//its the short cut of the iteroto.  method has while loop (yukardaki) on his own body in default
